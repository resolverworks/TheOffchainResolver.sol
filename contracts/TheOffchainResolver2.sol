...
contract TheOffchainResolver is IERC165, ITextResolver, IAddrResolver, IAddressResolver, IPubkeyResolver, IContentHashResolver, IMulticallable, IExtendedResolver, IExtendedDNSResolver, IOnchainResolver {
	...
	uint256 constant COIN_TYPE_FALLBACK_NODE = 0xb32cdf4d3c016cb0f079f205ad61c36b1a837fb3e95c70a94bdedfca0518a010; // https://adraffy.github.io/keccak.js/test/demo.html#algo=keccak-256&s=fallback&escape=1&encoding=utf8
	...
	function resolve(bytes calldata name, bytes calldata data) external view returns (bytes memory result) {
		unchecked {
			...
			while (true) {
				...
				if (ENS(ENS_REGISTRY).resolver(node0) == address(this)) break; // find our name
	...
	function determineExternalFallback(bytes32 node) internal view returns (bytes32 extnode, address resolver) {
		uint256 slot = slotForCoin(node, COIN_TYPE_FALLBACK_NODE);
		assembly { extnode := sload(slot) }
		if (extnode == 0) { 
			// try: namehash("_" + node)
